...
def mostrar_menu(root):
    ...
    roles_sesion["atacante"]  = 1
     ...
     # Canvas como fondo en vez de Frame
    ...
    try:
        ...
        canvas_menu.configure(bg="#1a1a2e")  # Fondo oscuro de respaldo si falla

    # Título encima de la imagen
    canvas_menu.create_text(
        400, 150,
        text="Asedio y defensa",
        font=("Arial", 28, "bold"),
        fill="#ffffff"
    )
    canvas_menu.create_text(
        400, 195,
        text="Juego de estrategia",
        font=("Arial", 12),
        fill="#cccccc"
    )

    # Botones flotando sobre la imagen usando create_window
...
def mostrar_login(root, numero_jugador=1):
    ...
    canvas = tk.Canvas(root, width=800, height=600, highlightthickness=0, bd=0)
    canvas.pack(expand=True, fill="both")
    try:
        img = Image.open("Menu.png").resize((800, 600), Image.LANCZOS)
        foto = ImageTk.PhotoImage(img)
        canvas.create_image(0, 0, anchor="nw", image=foto)
        canvas._fondo_ref = foto
        canvas.create_rectangle(0, 0, 800, 600, fill="#000000", outline="", stipple="gray50")
    except:
        canvas.configure(bg="#1a1a2e")

    canvas.create_text(400, 110, text=f"Jugador {numero_jugador} — Iniciar sesión",
                       font=("Arial", 22, "bold"), fill="#e0e0e0")
    canvas.create_text(400, 175, text="Usuario:", font=("Arial", 12), fill="#e0e0e0")
    entry_usuario = tk.Entry(root, font=("Arial", 12), width=25)
    canvas.create_window(400, 200, window=entry_usuario)
    canvas.create_text(400, 235, text="Contraseña:", font=("Arial", 12), fill="#e0e0e0")
    entry_contrasena = tk.Entry(root, font=("Arial", 12), width=25, show="*")
    canvas.create_window(400, 260, window=entry_contrasena)

    lbl_mensaje = tk.Label(root, text="", font=("Arial", 10), bg="#0d0d1a", fg="#ff6b6b")
    canvas.create_window(400, 295, window=lbl_mensaje)
    ...
    def intentar_login():
        ...
        if numero_jugador == 1:
            root.after(800, lambda: mostrar_login(root, 2))
        else:
            root.after(800, lambda: mostrar_facciones(root, 1))
    ...
    for texto, cmd, cy in [
        ("Iniciar sesión", intentar_login,             340),
        ("Registrarse",    intentar_registro,           410),
        ("Volver al menú", lambda: mostrar_menu(root),  480),
    ]:
        btn = tk.Button(root, text=texto, command=cmd,
                        font=("Arial", 14), bg="#16213e", fg="#e0e0e0",
                        activebackground="#0f3460", activeforeground="#ffffff",
                        width=20, height=2, bd=0, cursor="hand2", relief="flat")
        canvas.create_window(400, cy, window=btn)
...
def mostrar_facciones(root, numero_jugador=1):
    ...
    canvas.create_text(400, 100, text=f"Jugador {numero_jugador} — Elige tu facción",
                       font=("Arial", 22, "bold"), fill="#e0e0e0")
    if numero_jugador == 2 and facciones_sesion[0]:
        canvas.create_text(400, 140, text=f"Jugador 1 eligió: {facciones_sesion[0]}",
                           font=("Arial", 11), fill="#aaaaaa")

    lbl_mensaje = tk.Label(root, text="", font=("Arial", 10), bg="#0d0d1a", fg="#ff6b6b")
    canvas.create_window(400, 170, window=lbl_mensaje)
    ...
    for nombre, cy in zip(["Medieval", "Futurista", "Acuático"], [240, 320, 400]):
        btn = tk.Button(root, text=f"{nombre}\n{descripciones[nombre]}",
                        command=lambda f=nombre: elegir_faccion(f),
                        font=("Arial", 12), bg=colores[nombre], fg="#ffffff",
                        activebackground="#333333", width=30, height=3,
                        bd=0, cursor="hand2", relief="flat")
        canvas.create_window(400, cy, window=btn)

    btn_v = tk.Button(root, text="Volver al menú", command=lambda: mostrar_menu(root),
                      font=("Arial", 14), bg="#16213e", fg="#e0e0e0",
                      activebackground="#0f3460", activeforeground="#ffffff",
                      width=20, height=2, bd=0, cursor="hand2", relief="flat")
    canvas.create_window(400, 500, window=btn_v)
...
def mostrar_roles(root):
    ...
    canvas.create_text(400, 160, text="Asignación de roles",
                       font=("Arial", 22, "bold"), fill="#e0e0e0")
    canvas.create_text(400, 210, text="¿Quién jugará como Defensor?",
                       font=("Arial", 13), fill="#aaaaaa")
    ...
    for texto, cmd, cy in [
        (f"{j1}\n🛡  Defensor", lambda: elegir_defensor(0), 300),
        (f"{j2}\n🛡  Defensor", lambda: elegir_defensor(1), 380),
    ]:
        btn = tk.Button(root, text=texto, command=cmd,
    ...
    btn_v = tk.Button(root, text="Volver al menú", command=lambda: mostrar_menu(root),
                      ...
                      activebackground="#0f3460", activeforeground="#ffffff",
                      width=20, height=2, bd=0, cursor="hand2", relief="flat")
    canvas.create_window(400, 480, window=btn_v)
...
def mostrar_seleccion_mapa(root):
    ...
    canvas.create_text(400, 200, text="Elige el mapa",
                       font=("Arial", 22, "bold"), fill="#e0e0e0")
    ...
    for texto, tipo, cy, color in [
        ("Mapa Clásico", "clasico", 310, "#1d4d3a"),
        ("Mapa Libre",   "libre",   390, "#4a3728"),
    ]:
        btn = tk.Button(root, text=texto, command=lambda t=tipo: elegir_mapa(t),
                        font=("Arial", 14), bg=color, fg="#ffffff",
...
#CAMBIO
    ...
    try:
        ...
        canvas._fondo_tablero_ref = foto_tablero
    except:
        pass
    ...
    def dibujar_celda(fila, col):
        ...
        # ── Dibuja el fondo de la celda ── CAMBIO
        if color in ("#3a3a3a", "#2d2d2d"):
            canvas.create_rectangle(x1, y1, x2, y2,
                                     fill="#000001", outline="#444444",
                                     stipple="gray12",
